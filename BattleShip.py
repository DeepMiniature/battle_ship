...
bkrd = ImageTk.PhotoImage(Image.open(('Checkers_bs.png')))
back_img = ImageTk.PhotoImage(Image.open(('Back.png')))
#play_button = ImageTk.PhotoImage(Image.open(('play_btn.png')))
label_back = Label(screen, image = back_img).grid(row = 0, column = 0, columnspan = 700, rowspan = 700)
...
def change(var):
    ...
    if play=='yes' and var not in player_done:
        if var+1 in cship_pos1 or var+1 in cship_pos2 or var+1 in cship_pos3 or var+1 in cship_pos4 or var+1 in cship_pos5 :
            blocks[var] = Button(screen, bg='red', height=2, width=4).grid(row=(var // 10) * 50 + 30,column=((var) % 10) * 50 + 10, rowspan=50,columnspan=50)
            count_game_player+=1
            play = 'naa'
        else:
            blocks[var] = Button(screen, bg='white', height=2, width=4).grid(row=(var // 10) * 50 + 30,column=((var) % 10) * 50 + 10, rowspan=50,columnspan=50)
            play = 'naa'
    ...
    if play == 'naa':
        ...
        if num in pos1 or num in pos2 or num in pos3 or num in pos4 or num in pos5:
            blocks1[num] = Button(screen, bg='red', height=2, width=4).grid(row=(num // 10) * 50 + 30,column=((num) % 10) * 50 + 840, rowspan=50,columnspan=50)
            play = 'yes'
            count_game_comp+=1
        else:
            blocks1[num] = Button(screen, bg='white', height=2, width=4).grid(row=(num // 10) * 50 + 30,column=((num) % 10) * 50 + 840, rowspan=50,columnspan=50)
            play = 'yes'
        comp_done.append(num)

    if count_game_comp == 17:
        over_label=Label(screen, text='GAME OVER --> COMPUTER WINS').grid(row = 0 , column = 0, columnspan =1400, rowspan = 700)
        restart = Button(screen, text='Rematch???', command = start).grid(row = 0, column = 0, rowspan = 700, columnspan =700)
    if count_game_player == 17:
        over_label = Label(screen, text='GAME OVER --> PLAYER WINS').grid(row=0, column=0, columnspan=1400, rowspan = 700)
        restart = Button(screen, text='Rematch???', command = start).grid(row=0, column=700, rowspan=700, columnspan=700)
...
def ships(var):
    ...
    if rt.get()==1 and count1<5 and var not in pos1 and var not in pos2 and var not in pos3 and var not in pos4 and var not in pos5:
        if len(pos1)==0:
            blocks1[var] = Button(screen, bg='blue', height=2, width=4).grid(row=(var // 10) * 50 + 30,column=((var) % 10) * 50 + 840, rowspan=50,columnspan=50)
            pos1.append(var)
            count1+=1
        elif len(pos1)==1:
    ...
    if rt.get()==2 and count2<4 and var not in pos1 and var not in pos2 and var not in pos3 and var not in pos4 and var not in pos5:
        if len(pos2)==0:
            blocks1[var] = Button(screen, bg='brown', height=2, width=4).grid(row=(var // 10) * 50 + 30,column=((var) % 10) * 50 + 840, rowspan=50,columnspan=50)
            pos2.append(var)
            count2+=1
        elif len(pos2)==1:
    ...
    if rt.get()==3 and count3<3 and var not in pos1 and var not in pos2 and var not in pos3 and var not in pos4 and var not in pos5:
        if len(pos3)==0:
            blocks1[var] = Button(screen, bg='yellow', height=2, width=4).grid(row=(var // 10) * 50 + 30,column=((var) % 10) * 50 + 840, rowspan=50,columnspan=50)
            pos3.append(var)
            count3+=1
        elif len(pos3)==1:
    ...
    if rt.get()==4 and count4<3 and var not in pos1 and var not in pos2 and var not in pos3 and var not in pos4 and var not in pos5:
        if len(pos4)==0:
            blocks1[var] = Button(screen, bg='green', height=2, width=4).grid(row=(var // 10) * 50 + 30,column=((var) % 10) * 50 + 840, rowspan=50,columnspan=50)
            pos4.append(var)
            count4+=1
        elif len(pos4)==1:
    ...
    if rt.get()==5 and count5<2 and var not in pos1 and var not in pos2 and var not in pos3 and var not in pos4 and var not in pos5:
        if len(pos5)==0:
            blocks1[var] = Button(screen, bg='violet', height=2, width=4).grid(row=(var // 10) * 50 + 30,column=((var) % 10) * 50 + 840, rowspan=50,columnspan=50)
            pos5.append(var)
            count5+=1
        elif len(pos5)==1:
...
def start():
    global blocks, blocks1, rt, rb1, rb2, rb3, rb4, rb5
    var_assign()
    label_back1 = Label(screen, image=back_img)
    label_back1.grid(row=0, column=0, columnspan=700, rowspan=700)
    label_back2 = Label(screen, image=back_img)
    label_back2.grid(row=0, column=700, columnspan=700, rowspan=700)
    head_1 = Label(screen, text='PLAYER').grid(row=30, column = 620, rowspan = 150 )
    head_2 = Label(screen, text='COMPUTER').grid(row=30, column=800, rowspan = 150)

    bkrd_label = Label(screen, image = bkrd)
    bkrd_label.grid(row = 30, column = 10, rowspan = 551, columnspan = 550)
    ...
    for i in range(0,100):
        ...
        blocks[i].grid(row = (i//10)*50+30, column = ((i)%10)*50+10, rowspan = 50, columnspan = 50)
        blocks[i].configure(command = lambda  b=i: change(b))

    bkrd_label1 = Label(screen, image=bkrd)
    bkrd_label1.grid(row=30, column=840, rowspan=551, columnspan=550)
    ...
    for i in range(0, 100):
        ...
        blocks1[i].grid(row=(i // 10) * 50 + 30, column=((i) % 10) * 50 + 840, rowspan=50, columnspan=50)
        blocks1[i].configure(command=lambda b=i: ships(b))

    rt = IntVar()
    rt.set(0)
    rb1 = Radiobutton(screen, text='5 unit ship',variable =rt, value = 1)
    rb2 = Radiobutton(screen, text='4 unit ship', variable=rt, value=2)
    rb3 = Radiobutton(screen, text='3 unit ship I', variable=rt, value=3)
    rb4 = Radiobutton(screen, text='3 unit ship II', variable=rt, value=4)
    rb5 = Radiobutton(screen, text='2 unit ship', variable=rt, value=5)
    rb1.grid(row=180, column=700, columnspan=140, rowspan=150)
    rb2.grid(row=230, column=700, columnspan=140, rowspan=150)
    rb3.grid(row=280, column=700, columnspan=140, rowspan=150)
    rb4.grid(row=330, column=700, columnspan=140, rowspan=150)
    rb5.grid(row=380, column=700, columnspan=140, rowspan=150)
    on_btn=Button(screen, text='START GAME', command = comp_ships)
    on_btn.grid(row = 30, column = 0, rowspan= 150, columnspan =1400)


game_lab = Label(screen, text = 'Battle Ship').grid(row = 100, column = 0, columnspan = 700, rowspan = 700)
Start_button = Button(screen, text = 'START', command = start)
Start_button.grid(row = 300, column = 0, columnspan = 700, rowspan = 700)
